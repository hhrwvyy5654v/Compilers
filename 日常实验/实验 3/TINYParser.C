...
{
    struct treeNode *child[MAXCHILDREN]; //�ӽڵ�
    struct treeNode *sibling;            //�ֵܽڵ�
    int lineno;                          //��ǰ�ڵ����ڵ�����
    NodeKind nodekind;                   //��ǰ�ڵ������
    union
    {
        StmtKind stmt;
        ExpKind exp;
...
TreeNode *newExpNode(ExpKind kind)
{
    ...
    else
    {
        ...
        t->lineno = lineno;
        t->type = Void;
        if (token == NUM && kind == ConstK)
        {
            t->type = Integer;
        }
        if (token == STR && kind == ConstK)
        {
            t->type = String;
        }
    }
...
{
    TreeNode *root = newStmtNode(PROGRAMK);
